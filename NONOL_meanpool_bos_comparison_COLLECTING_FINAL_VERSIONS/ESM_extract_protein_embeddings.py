...
def parse_arguments():
    ...
    # BOS extraction mode
    parser.add_argument("--extract_bos", type=int, default=0, choices=[0, 1],
                        help="If 1, also extract BOS token representations (saves separate file)")
...
def extract_mean_pooled_esm_activations(
    ...
    Optionally also extract BOS token activations.
    ...
    Args:
        ...
        extract_bos: If True, also extract BOS token representations
    
    Returns:
        Tuple of:
        - Dictionary mapping layer_idx -> mean-pooled activation vector [d_model]
        - Dictionary mapping layer_idx -> BOS token activation vector [d_model] (or None if extract_bos=False)
    """
    ...
    layer_activations = {}
    bos_activations = {} if extract_bos else None
    seq_len = len(sequence)
    
    for layer_idx in layers_to_extract:
        full_repr = results["representations"][layer_idx][0]  # [seq_len_with_special, d_model]
        
        # Extract BOS token (position 0) if requested
        if extract_bos:
            bos_emb = full_repr[0]  # [d_model]
            bos_activations[layer_idx] = bos_emb.cpu()
        
        # Remove BOS and EOS tokens: [1:seq_len+1]
        embedding = full_repr[1:seq_len+1]  # [seq_len, d_model]
    ...
    return layer_activations, bos_activations
...
def main():
    ...
    print(f"Extract BOS: {'Yes' if args.extract_bos == 1 else 'No'}")
    print("=" * 60)
    ...
    bos_activations_dict = {layer_idx: [] for layer_idx in layers_to_extract} if args.extract_bos else None
    processed_labels = []
    processed_sequences = []
    ...
    for i, (sequence, label) in enumerate(tqdm(zip(sequences, labels), total=len(sequences), desc="Extracting activations")):
        try:
            # Extract mean-pooled activations for all layers (and optionally BOS)
            protein_layer_acts, protein_bos_acts = extract_mean_pooled_esm_activations(
                sequence, label, esm_model, alphabet, device, layers_to_extract,
                extract_bos=bool(args.extract_bos)
            )
            ...
            if args.relu == 1:
                for layer_idx in layers_to_extract:
                    ...
                    if args.extract_bos and protein_bos_acts is not None:
                        protein_bos_acts[layer_idx] = torch.relu(protein_bos_acts[layer_idx])
            
            # Store each layer's activation
            for layer_idx in layers_to_extract:
                layer_activations_dict[layer_idx].append(protein_layer_acts[layer_idx])
                if args.extract_bos and protein_bos_acts is not None:
                    bos_activations_dict[layer_idx].append(protein_bos_acts[layer_idx])
    ...
    for layer_idx in layers_to_extract:
        ...
        output_data = {
            ...
            'processing_info': {
                ...
                'representation_type': 'meanpool',  # Explicitly note this is mean-pooled
            }
        }
    ...
    # Save BOS activations if extracted
    if args.extract_bos and bos_activations_dict is not None:
        print(f"\nSaving BOS token activation files...")
        for layer_idx in layers_to_extract:
            # Stack all proteins' BOS activations for this layer
            bos_tensor = torch.stack(bos_activations_dict[layer_idx], dim=0)  # [n_proteins, d_model]
            
            # Prepare output data for this layer's BOS activations
            bos_output_data = {
                'embeddings': bos_tensor,
                'layer_idx': layer_idx,
                'processing_info': {
                    'esm_model': args.esm_model,
                    'layer_idx': layer_idx,
                    'max_seq_len': args.max_seq_len,
                    'embedding_dim': bos_tensor.shape[1],
                    'n_proteins_processed': len(processed_labels),
                    'timestamp': datetime.now().isoformat(),
                    'normalization_applied': False,
                    'relu_applied': bool(args.relu),
                    'representation_type': 'bos',  # Explicitly note this is BOS token
                }
            }
            
            # Add metadata if requested
            if args.save_metadata:
                bos_output_data['metadata'] = {
                    'labels': processed_labels,
                    'sequences': processed_sequences,
                }
                bos_output_data.update(stats)
            
            # Save this layer's BOS file
            bos_filename = f"layer_{layer_idx}_bos_activations.pt"
            bos_filepath = output_dir / bos_filename
            torch.save(bos_output_data, bos_filepath)
            
            file_size_mb = bos_filepath.stat().st_size / (1024*1024)
            print(f"  Layer {layer_idx:2d} BOS: {bos_tensor.shape} -> {bos_filename} ({file_size_mb:.2f} MB)")
    
    print("\n" + "=" * 60)
    print("Extraction complete!")
    print(f"Saved {len(layers_to_extract)} mean-pooled layer files to: {args.output_dir}")
    if args.extract_bos:
        print(f"Saved {len(layers_to_extract)} BOS token layer files to: {args.output_dir}")

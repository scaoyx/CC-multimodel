...
class NeuronDistanceAnalyzer:
    ...
    def __init__(self, tsv_path: str, activations_path: str,
        ...
        self.num_random_neurons = num_random_neurons
        self.x = x
        self.z = z
    ...
    def count_active_proteins(self, neuron_idx: int) -> int:
        """Count proteins with activation > 0 for a given neuron."""
        neuron_acts = self.activations[:, neuron_idx]
        return int(np.sum(neuron_acts > 0))
    ...
    def run(self):
        ...
        if self.random_mode:
            # Random mode: find neurons with enough active proteins
            min_active = self.x + self.z
            print(f"Random mode: looking for {self.num_random_neurons} neurons with at least {min_active} active proteins...")
            
            # Shuffle all neuron indices
            all_neurons = list(range(total_neurons))
            random.shuffle(all_neurons)
            
            selected = []
            for neuron_idx in all_neurons:
                if self.count_active_proteins(neuron_idx) >= min_active:
                    selected.append(neuron_idx)
                    if len(selected) >= self.num_random_neurons:
                        break
            
            print(f"Found {len(selected)} neurons with sufficient active proteins")
            if len(selected) < self.num_random_neurons:
                print(f"Warning: Only found {len(selected)} neurons with >= {min_active} active proteins (requested {self.num_random_neurons})")
        else:
...
def main():
    ...
    parser.add_argument("--neuron_file", default=None,
                        help="Path to a text file with one neuron index per line (required if not using --random_mode)")
    parser.add_argument("--random_mode", type=int, default=0,
                        help="Enable random neuron selection mode (1 to enable, 0 to disable)")
    parser.add_argument("--num_random_neurons", type=int, default=100,
                        help="Number of random neurons to analyze (only used with --random_mode)")
    ...
    if not random_mode and args.neuron_file is None:
        parser.error("--neuron_file is required when not using --random_mode")

    # read neuron list (only if not in random mode)
    neuron_indices = None
    if not random_mode:

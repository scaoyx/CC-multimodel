...
def extract_batch_activations_multimodel(
    ...
    """
    Extract layers 0, 1, 2 activations from multiple ESM models.
    ...
    for model_name, (esm_model, alphabet, device) in esm_models.items():
        ...
        # Extract only layer 1 (second layer)
        layers = [1]
...
def main():
    ...
    for idx, (model_name, model_loader) in enumerate(model_configs):
        ...
        layers_to_extract = [1]  # Extract only layer 1 (second layer)
        
        model_info[model_name] = {
            'num_layers': num_layers,
            'embed_dim': esm_model.embed_dim,
            'layers_extracted': layers_to_extract,
            'single_layer_extraction': True,
            'extracted_layer_index': 1,
            'device': device_id
        }
        
        print(f"    Layers: {num_layers}, Embed dim: {esm_model.embed_dim}")
        print(f"    Extracting only layer: {layers_to_extract[0]} (second layer)")
    ...
    config = {
        ...
        'extraction_mode': 'multimodel_single_layer_1',
    }

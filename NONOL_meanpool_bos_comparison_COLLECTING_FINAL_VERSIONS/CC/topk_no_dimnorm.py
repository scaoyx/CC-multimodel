#### CrossCoder WITHOUT dimensional normalization
#### This version does NOT normalize losses by dimensions - all sources contribute raw MSE
...
class TopKAuto(nn.Module):
    def __init__(self, input_dims, hidden_dim, k, encoder_decoder_init, 
        ...
        """
        Multi-modal TopK Autoencoder WITHOUT DIMENSIONAL NORMALIZATION
        ...
        Args:
            ...
            model_groups: Optional list mapping each source to its model index (for logging only)
        """
        ...
        # Store model groups (for logging, not for loss normalization)
    ...
    def forward(self, x):
        ...
        # Calculate reconstruction loss for all sources (NO DIMENSION NORMALIZATION)
        # Simply sum the raw MSE from each source without normalizing by n_sources
        main_loss = torch.zeros((), device=x[0].device)
        for i in range(self.n_sources):
            # Add raw MSE contribution from each source
            main_loss += torch.mean((x[i] - reconstructions[i]) ** 2)
        # NO DIVISION BY n_sources - larger-dimensional sources contribute more

        # Calculate per-source losses for logging (raw MSE)
        source_losses = []
        for i in range(self.n_sources):
            source_loss = torch.mean((x[i] - reconstructions[i]) ** 2)
            source_losses.append(source_loss)
        
        # Calculate per-model losses WITHOUT normalization (just sum raw MSE per model)
        model_losses = []
        if self.model_groups is not None and len(self.model_groups) > 0:
            # Compute raw MSE sum for each model (no averaging)
            ...
            # Return raw sum per model (NO NORMALIZATION by number of sources per model)
            for model_idx in range(self.n_models):
                model_losses.append(model_mse_sum[model_idx])
        ...
        if torch.count_nonzero(inactive_mask).item() != 0 and self.training:
            ...
            # Calculate residual for each source (NO NORMALIZATION)
            ...
            # NO DIVISION BY n_sources
            aux_pre_alpha = aux_loss_sum
...
class LitLit(pl.LightningModule):
    def __init__(self, input_dims, hidden_dim, k, encoder_decoder_init, 
        ...
        """
        Lightning module for multi-modal TopK autoencoder WITHOUT DIMENSIONAL NORMALIZATION
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Log per-model losses (no normalization)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Log per-model validation losses (no normalization)
        for i, model_loss in enumerate(model_losses):
            model_name = self.model_names[i] if self.model_names and i < len(self.model_names) else f'model_{i}'

...
def setup_layer_normalization(args, esm_model, alphabet, device) -> Optional[Dict]:
    ...
    if args.bos_vs_meanpool:
        ...
        print(f"Setting up layer normalization for BOS vs mean-pool mode (layer {target_layer})")
    else:
    ...
    if args.norm_scalars_path and Path(args.norm_scalars_path).exists():
        ...
        norm_computer = LayerNormalizationComputer(
            ...
            args.target_layer if args.bos_vs_meanpool else None
        )
...
def extract_esm_activations_for_protein(
    ...
    Returns:
        - If bos_vs_meanpool: [1, 2, d_model] where source 0 = BOS, source 1 = mean-pool
        - If mean_pooled: [1, n_sources, d_model]
        - Otherwise: [seq_len, n_sources, d_model]
    """
    ...
    if bos_vs_meanpool:
        ...
        # Extract activations for the target layer only
        with torch.no_grad():
            results = esm_model(
        ...
        full_repr = results["representations"][target_layer][0]  # [seq_len_with_special, d_model]
        ...
        token_emb = full_repr[1:seq_len+1]  # [L, d_model]
        meanpool_emb = token_emb.mean(dim=0)  # [d_model]
        ...
        # Stack to create [1, 2, d_model]
        activations = torch.stack([bos_emb, meanpool_emb], dim=0).unsqueeze(0)
        return activations
...
def main():
    ...
    if args.bos_vs_meanpool:
        print(f"Target layer: {args.target_layer if args.target_layer is not None else 'last layer (default)'}")
    ...
    if args.bos_vs_meanpool:
        # BOS vs mean-pool mode: single layer, 2 sources
        target_layer = args.target_layer
        if target_layer is None:
            target_layer = esm_model.num_layers - 1
        layers_to_extract = [target_layer]
        print(f"BOS vs mean-pool mode: extracting from layer {target_layer}")
        print(f"  Source 0: BOS token representation")
        print(f"  Source 1: Mean-pooled token representations")
        
        # Verify crosscoder expects 2 sources
        expected_n_sources = len(crosscoder_model.hparams.input_dims)
        if expected_n_sources != 2:
            raise ValueError(
                f"BOS vs mean-pool mode expects crosscoder with 2 sources, "
                f"but checkpoint has {expected_n_sources} sources. "
                f"Make sure you're using a checkpoint trained with --bos_vs_meanpool 1"
            )
    else:
        if args.layers_to_extract is None:
            layers_to_extract = list(range(esm_model.num_layers + 1))
    ...
    for i, (sequence, label) in enumerate(tqdm(zip(sequences, labels), total=len(sequences), desc="Processing proteins")):
        try:
            ...
            esm_activations = extract_esm_activations_for_protein(
                ...
                bool(args.bos_vs_meanpool), args.target_layer if args.bos_vs_meanpool else None
            )
            # esm_activations shape: 
            # - bos_vs_meanpool: [1, 2, d_model]
            # - mean_pooled: [1, n_sources, d_model]
            # - otherwise: [seq_len, n_sources, d_model]
            
            # Pass through crosscoder to get hidden activations
            hidden_acts = extract_hidden_activations_from_crosscoder(
                esm_activations, crosscoder_model, bool(args.mean_pooled) or bool(args.bos_vs_meanpool)
            )
            # hidden_acts shape: [seq_len, hidden_dim] or [1, hidden_dim]
            
            # Pool to get single vector per protein
            if bool(args.mean_pooled) or bool(args.bos_vs_meanpool):
    ...
    output_data = {
        ...
        'processing_info': {
            ...
            'target_layer': args.target_layer if args.bos_vs_meanpool else None,
